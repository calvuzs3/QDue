...
import net.calvuz.qdue.ui.shared.base.BaseFragmentLegacy;
...
/**
 * Migration helper that extends existing BaseFragmentLegacy with virtual scrolling capabilities
 * This allows gradual migration of existing fragments
 */
public abstract class EnhancedBaseFragmentLegacyBridge extends BaseFragmentLegacy {

    private static final String TAG = "BF-Bridge";

    protected AdapterLegacyBridge adapterBridge;
    ...
    protected void loadInitialVirtualData() {
        if (adapterBridge != null) {
            // ✅ Access VirtualDataManager through AdapterLegacyBridge
            adapterBridge.requestInitialData();

            Log.d(TAG, "loadInitialVirtualData: ✅ Requesting initial virtual data via AdapterLegacyBridge");
        } else {
            Log.e(TAG, "loadInitialVirtualData: ❌ AdapterLegacyBridge is null - cannot load virtual data");
        }
    }
    ...
    private void setupHybridVirtualScrolling() {
        ...
        try {
            // ✅ 1. Reset flags (from BaseFragmentLegacy)
            resetControlFlags();
            ...
            // Virtual mode doesn't need full cache, but BaseFragmentLegacy expects some items
    ...
    /**
     * Generate minimal cache items for BaseFragmentLegacy compatibility
     * Virtual adapter will override these, but BaseFragmentLegacy needs non-empty cache
     */

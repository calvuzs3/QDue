...
public class ShiftTypeFactory {
    ...
    private static final Map<String, LegacyShiftType> dynamicCache = new ConcurrentHashMap<>();
    private static final Map<Integer, LegacyShiftType> indexCache = new ConcurrentHashMap<>();
     ...
     *
     * @param index LegacyShift index (0-based)
     * @return LegacyShiftType or null if not found
     */
    public static LegacyShiftType getShiftType(int index) {
        ensureInitialized();
     ...
     *
     * @param name LegacyShift name
     * @return LegacyShiftType or null if not found
     */
    public static LegacyShiftType getShiftType(String name) {
        ensureInitialized();
     ...
     */
    public static List<LegacyShiftType> getAllShiftTypes() {
        ensureInitialized();
        List<LegacyShiftType> result = new ArrayList<>();
        for (int i = 0; i < currentShiftCount; i++) {
            LegacyShiftType shift = indexCache.get(i);
            if (shift != null) {
                result.add(shift);
            }
        }
     ...
     *
     * @param legacyShiftType LegacyShift type to add
     * @return Index assigned to the shift type
     */
    public static int addShiftType(LegacyShiftType legacyShiftType) {
        if (legacyShiftType == null) return -1;

        int index = currentShiftCount;
        dynamicCache.put( legacyShiftType.getName(), legacyShiftType );
        indexCache.put(index, legacyShiftType );
        currentShiftCount++;

        Log.d(TAG, "Added shift type: " + legacyShiftType.getName() + " at index " + index);
        return index;
    }
     ...
     * @param legacyShiftType New shift type
     * @return true if update was successful
     */
    public static boolean updateShiftType(int index, LegacyShiftType legacyShiftType) {
        if (index < 0 || index >= currentShiftCount || legacyShiftType == null) {
            return false;
        }

        // Remove old entry from name cache
        LegacyShiftType oldShift = indexCache.get(index);
        if (oldShift != null) {
        ...
        Log.d(TAG, "Updated shift type at index " + index + ": " + legacyShiftType.getName());
        return true;
    }
    ...
    public static int createAndAddShiftType(String name, String description,
                                            ...
                                            int color) {
        LegacyShiftType legacyShiftType = new LegacyShiftType(name, description, startHour, startMinute,
                durationHours, durationMinutes, color);
        return addShiftType( legacyShiftType );
    }
    ...
    private static void createDefaultShifts() {
        ...
        for (int i = 0; i < currentShiftCount && i < defaultNames.length; i++) {
            LegacyShiftType shift = new LegacyShiftType(
                    defaultNames[i],
                    defaultNames[i],
                    defaultStartHours[i],
                    0,
    ...
    private static boolean loadFromLocalStorage(Context context) {
        try {
            ...
            for (int i = 0; i < currentShiftCount; i++) {
                ...
                LegacyShiftType shift = new LegacyShiftType(name, description, startHour, startMinute,
                        durationHours, durationMinutes, color);

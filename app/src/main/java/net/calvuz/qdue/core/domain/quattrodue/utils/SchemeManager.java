...
public final class SchemeManager {
     ...
     *
     * @param legacyShiftTypes List of shift types
     * @return List of scheme days
     */
    public static List<Day> generateCycleDays(List<LegacyShiftType> legacyShiftTypes) {
     ...
     * @return LegacyShift index (0-based) if team is on shift, -1 otherwise
     */
    public static int findTeamShiftIndex(Day day, HalfTeam team) {
        ...
        for (int i = 0; i < legacyShifts.size(); i++) {
            if (legacyShifts.get(i).containsHalfTeam(team)) {
                return i;
            }
        }
    ...
    public static LocalDate findNextShiftDate(LocalDate startDate, HalfTeam team, int maxDaysToCheck) {
        ...
        try {
            ...
            // Old version
//            List<LegacyShiftType> legacyShiftTypes = new ArrayList<>();
//            for (int i = 0; i < SHIFTS_PER_DAY; i++) {
//                legacyShiftTypes.add(ShiftTypeFactory.createCustom(String.valueOf(i + 1),
//                        "LegacyShift " + (i + 1), 5 + i*8, 0, 8, 0));
//            }

            List<Day> cycleDays = generateCycleDays( legacyShiftTypes );
